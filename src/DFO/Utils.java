...
public class Utils {
	...
	public void findBestFly() {
		...
		for (int i = 1; i < Global.popSize; i++) {
			if (Global.fly[i].getFitness() <= min) { //min fitness
	...
	public double symmetry(double[] p) {
	    ...
	    randX = r.nextInt(maxX+1)-(2*maxX+1);
	    randY = r.nextInt(maxY+1)-(2*maxX+1);
	    ...
	    int sum1 = 0;
	    // Calculate the value of sum1 here, radxrad block of pixels x1y1 (top)
	    		for (int i = 0; i < rad; i++) {
	    			for (int j = 0; j < rad; j++)
	    			{
	    ...
	    // Calculate the value of sum2 here, radxrad block of pixels x2y2 (bottom)
			for (int i = 0; i < rad; i++) {
				for (int j = 0; j < rad; j++)
				{
					sum2 += Global.ss[torus(x2-i)][torus(y2-j)];
    					//System.out.println("sum1 = " + sum1);
				}
			}
